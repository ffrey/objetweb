...
foreach ($files AS $file) {
	...
	if ($db) {
		print 'svn deleting ' . $File->getFilename() . "\n\r";
	}
	// --force is used to force deletion upon modified files !
	$ok = exec("svn del --force " . $file, $output, $error);
	$ok = false;
	if ($error) {
		$erreurs[] = $error;
	}
